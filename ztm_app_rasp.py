...
def prepare_display_df(input_df, delay):
    tt = pd.read_csv('timetable.csv')
    ...
    input_df.time_diff.fillna(input_df.last_time_diff - delay, inplace=True)
    input_df['display_time'] = round(input_df['time_diff'] / 60, 1).astype(str) + " min"
...
def display_controller(df_display, direction_flag):
    if direction_flag == 1:
        ...
        title = 'Kierunek BRÓDNO'

    draw.rectangle((0,0,width,height), outline=0, fill=0) 
    draw.text((0,0), title, fill=255)
    
    for i, row in df_display.iterrows():
        draw.text((0,8*(i+1)), row['line']+' - KIERUNEK - '+str(row['display_time']), fill=255)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        sleepy_time = refresh_rate-(up_date - down_date).total_seconds()
                    
        init_df = prepare_display_df(init_df, sleepy_time)
        display_df = init_df.query("direction == 'going to station 2' and time_diff>=0 and after_me==0")
        display_df.reset_index(inplace=True, drop=True)
        display_controller(display_df, 1)
